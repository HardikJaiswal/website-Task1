class Employee{
  constructor(firstName,lastName,email,jobTitle,department,office,phoneNumber,skypeID,index){
...
function getEmloyeeDetails(staff){
  staff.firstName = getElementWithId("fName").value;
  staff.lastName = getElementWithId("lName").value;
  staff.email = getElementWithId("email").value;
  ...
  staff.phoneNumber = getElementWithId("phoneNumber").value.toString();
  staff.skypeID = getElementWithId("skypeID").value;
}

function createEmployee(){
  let staff = new Employee();
  getEmloyeeDetails(staff);
  if(staff.firstName!=="" && staff.lastName!=="" && staff.email!=="" && staff.phoneNumber!==""){
...
let index = 0;
let sample = new Employee("Hardik","Jaiswal","hardik@technovert.com","Intern","IT","India","7060232702","hj83700@skype",index++);
